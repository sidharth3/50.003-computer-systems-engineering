...
void main_loop(char *fileName)
{
    ...
    { //while the file still has input
        ...
        while (true)
        {
            for (int i = 0; i < number_of_processes; i++)
            {
                aliveFlag = waitpid(children_processes[i], NULL, WNOHANG);
                if (shmPTR_jobs_buffer[i].task_status == 0 && aliveFlag == 0)
                {
                    shmPTR_jobs_buffer[i].task_status = 1;
...
void cleanup()
{
    ...
    int detach = shmdt((void *)ShmPTR_global_data); //detach
    int remove= shmctl(ShmID_global_data, IPC_RMID, NULL); //delete
    detach = shmdt((void *)shmPTR_jobs_buffer); //detach
    remove = shmctl(ShmID_jobs, IPC_RMID, NULL); //delete
